...
class RootModel extends CI_Model{
	...
	public function get_allMACCount(){
		
	return $this->db->count_all('tbl_inventory');	
	}
	
	public function get_assignedMACCount(){
		
		$query = $this->db->query('SELECT * FROM tbl_inventory WHERE avaibility_status="assigned"');
		return $query->num_rows();
		
	}
	
	function downloadInventory_csv()
	{
        $this->load->dbutil();
        $this->load->helper('file');
        $this->load->helper('download');
        $query = $this->db->query("SELECT * FROM tbl_inventory");
        $delimiter = ",";
        $newline = "\r\n";
	...
	//GET MAC ID LIST FROM INVENTORY
	public function fetch_mac_id_list(){
	$sql = "SELECT id,predigit,device_id FROM `tbl_inventory` WHERE status = 'Active' AND avaibility_status = 'available'";
		$result = $this->db->query($sql);
		$arr_macid = array();
		foreach ($result->result() as $row) {
			$arr_macid[] = array(
				'id' => $row->id,
				'predigit' => $row->predigit,
