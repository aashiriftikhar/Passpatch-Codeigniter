...
class MemberModel extends CI_Model{
	...
	public function getRows($params = array()){
		...
		$this->db->select("tbl_members.id,member_name,room_number,floor_number,building_number,device_mac_id,event_id,status , tbl_event.event_name");

        $this->db->from($this->tableName);
        $this->db->join("tbl_event","tbl_event.id=tbl_members.event_id",'left');    
		...
		if(!empty($params['searchKeyword'])){
			...
			$this->db->where("(
                ...
                member_name LIKE '%".$params['searchKeyword']."%' OR
                room_number LIKE '%".$params['searchKeyword']."%' OR
                floor_number LIKE '%".$params['searchKeyword']."%' OR
                building_number LIKE '%".$params['searchKeyword']."%' OR
                device_mac_id LIKE '%".$params['searchKeyword']."%' OR
                event_name LIKE '%".$params['searchKeyword']."%' OR                
                status LIKE '%".$params['searchKeyword']."%')");
		}
        
        if(array_key_exists("id",$params)){
            $this->db->where('tbl_members.id',$params['id']);
			$query = $this->db->get();
